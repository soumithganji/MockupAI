    ...
    switch (element.type) {
      ...
      `;
        ...
        case 'navbar':
            ...
            const navIcons = { 'Home': 'home', 'Search': 'search', 'Cart': 'cart', 'Profile': 'user', 'Settings': 'settings', 'Chat': 'send', 'Help': 'bell' };

            return `
        <div class="wf-navbar" data-element-id="${id}">
          ${items.map((item, i) => `
            <div class="wf-nav-item ${i === 0 ? 'active' : ''}">
      ...
      `;
        ...
        <div class="wf-element wf-text wf-text-body ${selectedClass}" data-element-id="${id}">
          ${element.content || element.text || element.type}
        </div>
      `;
    }
}
